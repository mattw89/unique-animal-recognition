...
def loss_contrastive(output_1, output_2, tf_label, margin = 5.0):
    # Define loss function
    labels = tf_label
    # Euclidean distance squared
    eucd2 = tf.pow(tf.subtract(output_1, output_2), 2, name = 'eucd2')
...
#            tf.summary.scalar('Top-n Accuracy', topnacc)
    ...
    if args.transfer_mode[0]==3:
      ...
      train_op = tf.group(opt.apply_gradients(grads), tf.group(*batchnorm_updates))
    ...
    for epoch in xrange(start_epoch, start_epoch + args.num_epochs):
      ...
      for step in xrange(args.num_batches):
        ...
        _, loss_value = sess.run(
            [train_op, cross_entropy_mean], options= run_options, run_metadata= run_metadata)
        ...
        if step % 10 == 0:
          ...
          format_str = ('%s: epoch %d, step %d, loss = %.2f, (%.1f cats/sec; %.3f '
                        'sec/batch)')
...
def main():  # pylint: disable=unused-argument
    ...
    parser.add_argument('--batch_size', default= 64, type= int, action= 'store', help= 'The training batch size')
    ...
    parser.add_argument('--num_epochs', default= 4, type= int, action= 'store', help= 'The number of epochs')
    parser.add_argument('--path_prefix', default= './../../images/', action='store', help= 'the prefix address for images')
    parser.add_argument('--data_info', default= 'train_labels_siamese_mf.csv', action= 'store', help= 'Name of the file containing addresses and labels of training images')
    ...
    parser.add_argument('--snapshot_prefix', default= 'snapshots', action= 'store', help= 'Prefix for checkpoint files')
    ...
    parser.add_argument('--retrain_from', default='./snapshots/', action= 'store', help= 'Continue Training from a snapshot file')
    ...
    parser.add_argument('--transfer_mode', default = [1], nargs='+', type= int, help= 'Transfer mode 0=None , 1=Tune last layer only , 2= Tune all the layers')
